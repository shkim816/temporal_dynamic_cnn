...
parser.add_argument('--seed',           type=int,   default=2021,     help='Seed for the random number generator')
...
parser.add_argument('--decay_interval',  type=int,  default=15,      help='Learning rate decay every [decay_interval] epochs')
...
parser.add_argument('--save_path',      type=str,   default="exps/exp0", help='Path for model and logs')
...
parser.add_argument('--model',          type=str,   default="TDy_ResNet34_half",     help='Name of model definition')
parser.add_argument('--encoder_type',   type=str,   default="AVG",  help='Type of encoder')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.eval == True:
        ...
        for ii in range(10):
            trainer.__model__.module.update_temperature()
        ...
        if args.gpu == 0:
            ...
            print('\n###############################')
            print(time.strftime("%Y-%m-%d %H:%M:%S"), "VEER {:2.4f}".format(result[1]), "MinDCF {:2.5f}".format(mindcf))
            print('###############################')
    ...
    for it in range(it,args.max_epoch):
