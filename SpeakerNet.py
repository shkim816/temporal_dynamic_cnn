...
class SpeakerNet(nn.Module):
    ...
    def update_temperature(self):
        self.__S__.update_temperature()


class ModelTrainer(object):

    def __init__(self, speaker_model, optimizer, scheduler, gpu, mixedprec, model, **kwargs):
    ...
    def train_network(self, loader, verbose):
        ...
        for data, data_label in loader:
            ...
            if verbose:
                ...
                sys.stdout.write("Loss {:f} TEER/TAcc {:2.3f}% - {:.2f} sec ".format(loss/counter, top1/counter, telapsed))
                sys.stdout.flush()
        ...
        if self.model_name.startswith('TDy'):
            self.__model__.module.update_temperature()
    ...
    def loadParameters(self, path):
        ...
        loaded_state = torch.load(path, map_location="cuda:%d"%self.gpu)['model_state_dict']
        loaded_optimizer_state = torch.load(path, map_location="cuda:%d"%self.gpu)['optimizer_state_dict']
        self.__optimizer__.load_state_dict(loaded_optimizer_state)
